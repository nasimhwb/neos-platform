...
if [ -n "${POSTGRES_MULTIPLE_DATABASES:-}" ]; then
    ...
    for db_entry in "${DB_LIST[@]}"; do
        ...
        if [ -n "$db_entry" ]; then
            ...
            timeout 180s docker exec -i neos_postgres pg_dump -U "$POSTGRES_SUPERUSER" "$db_name" | gzip > "$SESSION_DIR/postgres_$db_name.sql.gz"
        fi
    done
else
    echo "No databases defined in POSTGRES_MULTIPLE_DATABASES, dumping only main..."
    timeout 180s docker exec -i neos_postgres pg_dump -U "$POSTGRES_SUPERUSER" "$POSTGRES_SUPERUSER" | gzip > "$SESSION_DIR/postgres_main.sql.gz"
fi
