...
|---|---|---|---|---|---|---|
| `/api/tasks` returns `404 Profile not found` | RESOLVED IN CODE (REDEPLOY PENDING) | `GET /api/tasks` returns 404; PostgREST profile returns 200 | `dashboard/Dockerfile` lacked `ARG NEXT_PUBLIC_SUPABASE_URL` during `next build` | Injected build ARGs & env vars into `dashboard/Dockerfile` and `compose.dashboard.yml` | DevOps / App Team | 2026-07-24 |
