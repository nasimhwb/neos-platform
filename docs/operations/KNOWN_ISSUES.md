...
| ISSUE-009 | Ingress / Auth | Traefik 404 on `supabase.neosfacility.com` causing `AuthUnknownError` (position 4) | FIX COMMITTED (Awaiting VPS Git Fast-Forward) | `curl -i https://supabase.neosfacility.com/auth/v1/health` returns HTTP 404 plaintext `404 page not found`; `Host: supabase.neos-platform.local` returns HTTP 200 OK via Kong/GoTrue | Traefik dynamic config lacked router rule for `Host(supabase.neosfacility.com)`. Client `JSON.parse` crashed on plaintext body. | Added `supabase-subdomain-router` to `configs/traefik/dynamic.yml` and pushed to `master` (Commit `21f187c`). Live VPS requires `git pull --ff-only origin master`. | SRE / DevOps | 2026-08-09 |
| ISSUE-010 | Ingress / Traefik | Traefik v3 Parser Error on Multi-Argument `Host()` Rules | RESOLVED (Commit pending push) | Traefik logs: `unexpected number of parameters; got 2, expected one of [1]` | Traefik v3 dynamic config contained legacy v2 `Host('a', 'b')` syntax across 5 routers | Migrated 5 routers in `configs/traefik/dynamic.yml` to Traefik v3 boolean expressions `(Host('a') \|\| Host('b'))` | SRE / DevOps | 2026-08-09 |
