...
| ISSUE-009 | Ingress / Auth | Traefik 404 on `supabase.neosfacility.com` causing `AuthUnknownError` (position 4) | DIAGNOSED (ROOT CAUSE PROVEN) | `curl -i https://supabase.neosfacility.com/auth/v1/health` returns HTTP 404 plaintext `404 page not found`; `Host: supabase.neos-platform.local` returns HTTP 200 OK via Kong/GoTrue | Traefik reverse proxy lacks router rule for `Host(supabase.neosfacility.com)` (`compose.supabase.yml` defaulted to `supabase.neos-platform.local` and `dynamic.yml` lacked rule). Client `JSON.parse` crashes on plaintext body. | Add `Host(supabase.neosfacility.com)` router rule to Traefik configuration (Awaiting Approval) | SRE / DevOps | 2026-08-09 |
